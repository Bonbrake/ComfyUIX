...
class QATestRunner:
    ...
    def test_gpu_doctor(self):
        ...
        try:
            ...
            is_valid_vram = (vram_mb >= 0) if (rec_mode == "CPU Mode" or info.get("vendor") in ("generic", "unknown")) else (vram_mb > 0)
            self.record_test(cat, "VRAM Detection", is_valid_vram, f"Detected VRAM: {info.get('vram_gb', 0)} GB ({vram_mb} MB)")
