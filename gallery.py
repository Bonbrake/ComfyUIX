...
def generate_pbr_maps(image_path: str) -> dict:
    ...
    try:
        with Image.open(image_path).convert("RGB") as im:
            ...
            # 2. Tangent-space Normal Map (Sobel or NumPy Gradient fallback)
            # Bolt optimization: scipy is not a required dependency; fallback to np.gradient for high performance & robustness
            if gray_arr is not None:
                try:
                    ...
                    # High-speed numpy gradient fallback: np.gradient computes central differences
