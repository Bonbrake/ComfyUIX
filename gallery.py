...
def generate_pbr_maps(image_path: str) -> dict:
    ...
    try:
        with Image.open(image_path).convert("RGB") as im:
            ...
            # Bolt Optimization: Use np.gradient fallback when scipy is absent to generate normal, roughness, height & AO maps cleanly.
            if gray_arr is not None:
                if sobel is not None:
                    dx = sobel(gray_arr, axis=1) * 3.0
                    dy = sobel(gray_arr, axis=0) * 3.0
                else:
                    dy_grad, dx_grad = np.gradient(gray_arr)
                    dx = dx_grad * 6.0
                    dy = dy_grad * 6.0
