        ...
        with Image.open(image_path).convert("RGB") as im:
            ...
            if gray_arr is not None:
                def _sobel(arr, axis):
                    ...
                    if axis == 1:
                        return (p[0:-2, 2:] + 2*p[1:-1, 2:] + p[2:, 2:] - (p[0:-2, 0:-2] + 2*p[1:-1, 0:-2] + p[2:, 0:-2]))
                    else:
                        return (p[2:, 0:-2] + 2*p[2:, 1:-1] + p[2:, 2:] - (p[0:-2, 0:-2] + 2*p[0:-2, 1:-1] + p[0:-2, 2:]))

                dx = _sobel(gray_arr, axis=1) * 3.0
                dy = _sobel(gray_arr, axis=0) * 3.0
