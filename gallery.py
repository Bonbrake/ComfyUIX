...
def generate_pbr_maps(image_path: str) -> dict:
    ...
    try:
        with Image.open(image_path).convert("RGB") as im:
            ...
            # 2. Tangent-space Normal Map (Pure numpy Sobel filter)
            if gray_arr is not None:
                # Sobel X: [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]
                dx = (np.roll(gray_arr, -1, axis=1) - np.roll(gray_arr, 1, axis=1)) * 2.0 + \
                     (np.roll(np.roll(gray_arr, -1, axis=0), -1, axis=1) - np.roll(np.roll(gray_arr, -1, axis=0), 1, axis=1)) + \
                     (np.roll(np.roll(gray_arr, 1, axis=0), -1, axis=1) - np.roll(np.roll(gray_arr, 1, axis=0), 1, axis=1))
                # Sobel Y: [[-1, -2, -1], [0, 0, 0], [1, 2, 1]]
                dy = (np.roll(gray_arr, -1, axis=0) - np.roll(gray_arr, 1, axis=0)) * 2.0 + \
                     (np.roll(np.roll(gray_arr, -1, axis=0), -1, axis=1) - np.roll(np.roll(gray_arr, 1, axis=0), -1, axis=1)) + \
                     (np.roll(np.roll(gray_arr, -1, axis=0), 1, axis=1) - np.roll(np.roll(gray_arr, 1, axis=0), 1, axis=1))
